...
Suppose that $\dfrac{d x(t)}{dt} =f(x(t))$, and that $x^\star$ is an equilibrium. Let $V(x)$ be a function defined in an open set $\mathcal O$ containing $x^\star$, such that a) $V(x^\star) = 0$ and $V(x) > 0$ for any $x \in \mathcal O, x \neq x^\star$, and b) $d V /dt \leq 0$ for all $x \in \mathcal O - x^\star$. Then $V$ is a *Lyapunov function* for the model, and $x^\star$ is stable. The Lyapunov function is called *strict*, and the equilibrium $x^\star$ *asymptotically stable* if $d V /dt < 0$ (i.e., all trajectories starting in $\matcal O$ will eventually reach $x^\star$).
