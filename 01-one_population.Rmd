...
where the independent variable $t$ denotes time, and is considered non-negative. The vector $x(t)$ tracks the state of the system (e.g., the density of the species) at time $t$, $x(t) \in \mathbb{R}^n$, and the function $f$ models the evolution of the system, mapping $\mathbb{R}^n \to \mathbb{R}^n$. The vector $x_0 = x(0)  \in \mathbb{R}^n$ marks the *initial condition* of the system.
...
When we evaluate this function at the equilibrium $x^\star = 0$, we obtain $\rho > 0$, and as such the equilibrium is unstable. Conversely, when we evaluate the fuction at $x^\star = 1 / \alpha$, we obtain $\rho  - 2 \rho = -\rho$ and therefore the equilibrium is stable.
...
Suppose that $\dfrac{d x(t)}{dt} =f(x(t))$, and that $x^\star$ is an equilibrium. Let $V(x)$ be a function defined in an open set $\mathcal O$ containing $x^\star$, such that a) $V(x^\star) = 0$ and $V(x) > 0$ for any $x \in \mathcal O, x \neq x^\star$, and b) $d V /dt \leq 0$ for all $x \in \mathcal O - x^\star$. Then $V$ is a *Lyapunov function* for the model, and $x^\star$ is stable. The Lyapunov function is called *strict*, and the equilibrium $x^\star$ *asymptotically stable* if $d V /dt < 0$ (i.e., all trajectories starting in $\mathcal O$ will eventually reach $x^\star$).
