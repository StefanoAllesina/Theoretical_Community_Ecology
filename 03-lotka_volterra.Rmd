...
$$
V(x(t)) = 1^T C \left( x(t) -D(x^\star) -D(x^\star) \log D(x(t) / x^\star)\right)
$$
...
MacArtur considered a system with two classes of equations: those describing the dynamics of consumers $(x_i)$ and resources ($y_i$). Resources do not interact with each other, and consumers interact only through the sharing of resources:
...
Any matrix can be rewritten as a series of smaller matrices stitched together. For squared matrices, it is often convenient to partition a matrix into blocks such that diagonal blocks are square matrices and off-diagonal blocks are (in general) rectangular. 
...
Similarly, if $A_{22}$ is invertible, and $\det(A) \neq 0$ (and hence the [Schur complement](https://en.wikipedia.org/wiki/Schur_complement) $A_{11} - A_{12}A_{22}^{-1} A_{21}$ is nonsingular) then
...
Next, we prove that if a feasible equilibrium for the system exists, it is globally stable. Again, we follow @case1979global to show that $A$ is indeed Lyapunov-diagonally stable. Consider:
