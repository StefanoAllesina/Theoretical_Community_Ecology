...
$$
\begin{align}
\frac{d V(z(t))}{dt} &= 1^T G \left(\frac{d z(t)}{dt}  + D(z^*) \frac{d \log z(t)}{dt} \right)\\
...
Which is again GLV, with "growth rates" $s = P^T\, D(b)^{-1} r - D(v)^{-1}m$ and "interactions" $B = P^T D(b)^{-1} P$, which importantly is symmetric.
