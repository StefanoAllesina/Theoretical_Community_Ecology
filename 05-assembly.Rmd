...
```{r, cowles, echo=FALSE,out.width = "35%"}
# All defaults
knitr::include_graphics("img/Cowles.jpg")
```
...
Following @hofbauer1998evolutionary (section 15.3), we assume that all species $x_i > 0$, and write:

$$
V(x) = \sum_i c_i (x_i - \bar{x_i} -\bar{x}_i \log x_i / \bar{x_i} )
$$

where for simplicity, we take $0 \log{x /0} = 0$. Note that this is always positive whenever $x \neq \bar{x}$.

We derive with respect to time, obtaining:

$$
\begin{aligned}
\dfrac{dV(x)}{dt} &= \sum_i c_i \left(\frac{d x_i}{dt} - \bar{x}_i \frac{d \log x_i}{dt} \right) \\
&= \sum_i c_i \left( x_i - \bar{x}_i \right)  \left( r_i + \sum_j A_{ij} x_j \right) \\
&= \sum_i c_i \left( x_i  - \bar{x}_i \right)  \left( r_i + \sum_j A_{ij} (x_j - \bar{x}_j) +  \sum_j A_{ij} \bar{x}_j \right) \\
&= \sum_{i,j} \left( x_i  - \bar{x}_i \right) c_i A_{ij} \left( x_j - \bar{x}_j \right) + \sum_i c_i \left( x_i - \bar{x}_i\right) \left(r_i + \sum_j A_{ij} \bar{x}_j \right)
\end{aligned}
$$

The first term is negative for all $x \neq \bar{x}$ (because $CA$ is negative definite), and is zero at the saturated equilibrium point (because $x = \bar{x}$). 

The second term is zero for all $\bar{x}_i > 0$: $r_i + \sum_j A_{ij} \bar{x}_j = 0$, given that $\bar{x}_i$ is an equilibrium for the species at positive density. For the remaining species (for which $\bar{x}_i = 0$), this amounts to the invasion criterion above, and must therefore be negative at the saturated equilibrium $\bar{x}$, yielding again a negative term. 

As such $V(x)$ is a Lyapunov function for the system, assuming negative values everywhere but at $\bar{x}$.
...
Now, let's simulate cases in which species do interact with each other, but $A$ is still D-stable. First, we load some functions to integrate GLV dynamics and plot results:
...
As you can see, the system still goes to the same equilibrium, with 3 and 5 extinct. Turns out, for this type of system, one does not even need to integrate dynamics: the Lemke–Howson algorithm can be adapted to solve the problem efficiently [@servan2018coexistence] (this is an efficient algorithm to find Nash equilibria in a game---we will hear more about this in the next lecture). I have coded up the algorithm already, and you can load the function `get_final_composition(A, r)` by typing:
...
This can be proven using a clever strategy devised by Carlos. To illustrate it, let's show that the probability of having all species coexisting is $1 / 2^n$. For this, we need $x^\star = -A^{-1}r$ to be feasible (then the equilibrium is reached because of the stability of $A$).

For each possible $A$ (invertible) and $r$, we have that the vector $- A^{-1}r$ will display a certain pattern of signs. Proving that the probability that all $n$ species coexist is $1 / 2^n$ amounts to proving that all sign patterns are equally probable. To this end, define the matrix $D_k = (-1)^{\delta_{ik}} \delta_{ij}$. This matrix is like the identity matrix, but with element $k$ of the diagonal set to $-1$ instead of $1$. We have that:
...
In fact, given enough time, any assembly history for a symmetric, stable matrix $A$ will eventually reach the final composition represented by the saturated equilibrium (@servan2020tractable).
...
```{r, echo=FALSE}
url <- "https://www.science.org/cms/10.1126/sciadv.aay8676/asset/5202f7c2-62ae-4f14-9348-ae19799842aa/assets/graphic/aay8676-f1.jpeg"
```
<center><img src="`r url`"></center>
