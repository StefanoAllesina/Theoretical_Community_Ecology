...
In particular, consider an ultrametric phylogenetic tree $T$, and the "variance-covariance" matrix induced by the tree, $\Sigma_T$. Because the tree is ultrametric, then $\Sigma_T_{ii} = 1$ for all $i$. The off-diagonal elements $\Sigma_T_{ij}$ express the proportion of shared ancestry (and as such $1\geq \Sigma_T_{ij} \geq 0$). 

Take the GLV model:

$$
\dfrac{dx}{dt}=D(x)(1 - \Sigma_T)
$$

Notice that $\Sigma_T$ is positive (semi-)definite by construction. Now build the assembly graph: does it have any interesting property?
