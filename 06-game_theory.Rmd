...
A mixed strategy $p$ is dominated by mixed strategy $\tilde{p}$ if $\tilde{p}^T A q \geq p^T A q$ for every $q$. The condition can be written as $(\tilde{p} - p)^T A q \geq 0$.

## Nash Equilibria

A pair of mixed strategies $p^\star$ and $q^\star$ is called a Nash Equilibrium for a two person matrix game if:
...
Consider a symmetric bimatrix game $A$ and a Nash equilibrium $\tilde{x}$. In the context of evolution, one might imagine that "mutants" are generated, playing a variation on $\tilde{x}$. A Nash equilibrium is *evolutionary stable* if it has a higher payoff than nearby mutants. The concept of an Evolutionary Stable Strategy (ESS) was introduced by Maynard-Smith and Price. 

We write a mutant as the sum of two strategies: the Nash equilibrium $\tilde{x}$, and an arbitrary strategy $x$: $y = (1 -\epsilon) \tilde{x} + \epsilon x$. The value $0\leq \epsilon \leq 1$ determines how close the mutant is to the Nash equilibrium. 
...
i.e., if all types have the same fitness $f_i = \bar{f}$ at $p^\star$. But then:
...
Expanding $\dot{p}$, we obtain:

$$
\dot{W} = 2 p^T A D(p) (Ap - p^T A p 1)
$$

Now we note that $x^T (Ax - x^T A x 1) = x^T A x - x^T A x x^T 1 = 0$.  
...
Very interestingly, the notion of ESS is not closed under the second invariant: we can have a $p^\star$ that is not an ESS, and choose $w > 0_n$ such that $q^\star$ is an ESS (and viceversa). I have recently wrote a paper exploiting this fact to extend the reach of the Lyapunov function above. 
